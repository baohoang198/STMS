...
% Temporary files to output script commands.
hfssExePath = 'C:\Program Files\AnsysEM\AnsysEM19.2\Win64\ansysedt.exe';    
hfssScriptFile = 'C:\Users\dell\Desktop\tmpFoldedDipoleR1.vbs';
tmpPrjFile = 'C:\Users\dell\Desktop\tmpFoldedDipoleR1.aedt';
...
% Create return loss report (new HFSS-API)
    hfssCreateReport1(fid,'ReturnLoss',1,1,'Setup150MHz','Sweep130to170MHz',...
    [],'Sweep',{'Freq'},{'Freq','db(S(Port1,Port1))'})   ;
     
% Export the S11 report to .tab file
    hfssExportToFile(fid, 'ReturnLoss', 'D:\CRD\practice\tmpFoldedDipoleR1', 'tab');    
    
% Close the HFSS Script File.
	fclose(fid);
    
    hfssExecuteScript(hfssExePath, hfssScriptFile);
    
% % Import the data
%     newData1 = importdata('C:\Users\dell\Desktop\tmpFoldedDipoleR1.tab'); 
%     Fre=newData1.data(:,1);         
%     ReturnLoss=newData1.data(:,2);
%   
% % Plot the data.
% 	disp('Solution Completed. Plotting Results for this Iteration ...');
% 	figure(1);
% 	hold on;
% 	plot(Fre, ReturnLoss,'red'); 
%     hold on;
